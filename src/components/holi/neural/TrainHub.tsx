...
export function TrainHub({
  ...
  return (
    <div className="relative z-20 grid w-full max-w-5xl gap-3 overflow-x-clip sm:gap-4 lg:grid-cols-[1.1fr_0.9fr]">
      <section className="border border-white/10 bg-black/35 p-4 backdrop-blur-sm sm:p-6">
        <p className="font-mono-code text-[0.6rem] tracking-[0.28em] text-[var(--holive-gold)]/75 uppercase">
          {labels.eyebrow}
        </p>
        <h3 className="mt-2 font-display text-xl text-[var(--holive-gold)] sm:text-4xl">
          {labels.title}
        </h3>
      ...
      <section className="border border-[var(--holive-gold)]/20 bg-[linear-gradient(160deg,rgba(201,168,76,0.12),rgba(0,0,0,0.35))] p-4 backdrop-blur-sm sm:p-6">
...
function ModeCard({
  ...
  return (
    ...
    >
      <span className="font-display text-lg text-[var(--holive-gold)] sm:text-xl">{title}</span>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="border border-white/10 bg-black/30 px-3 py-3">
      ...
      <p className="mt-1 font-display text-lg text-[var(--holive-gold)] sm:text-xl">{value}</p>
    </div>
  );
}
